...
public class DimensionTeleporter4 extends Teleporter {
	...
	public DimensionTeleporter4(WorldServer worldIn) {
		super(worldIn);
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? pupmod.a : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity p_77184_1_, double p_77184_2_, double p_77184_4_, double p_77184_6_,
		...
		if (this.destinationCoordinateCache.containsItem(j1)) {
			DimensionTeleporter4.PortalPosition portalposition = (DimensionTeleporter4.PortalPosition) this.destinationCoordinateCache
					.getValueByKey(j1);
			d3 = 0.0D;
			...
			for (l3 = l - short1; l3 <= l + short1; ++l3) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						if (this.worldServerInstance.getBlock(l3, i2, l1) == pupmod.portal4) {
							while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == pupmod.portal4) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				this.destinationCoordinateCache.add(j1,
						new DimensionTeleporter4.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if (this.worldServerInstance.getBlock(i - 1, j, k) == pupmod.portal4) {
				i4 = 2;
			}
			...
			if (this.worldServerInstance.getBlock(i, j, k - 1) == pupmod.portal4) {
				i4 = 3;
			}
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						this.worldServerInstance.setBlock(l3, i4, j4, flag ? pupmod.a : Blocks.air);
					}
				}
			}
		}
		...
		for (i3 = 0; i3 < 4; ++i3) {
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = -1; k3 < 4; ++k3) {
					...
					this.worldServerInstance.setBlock(l3, i4, j4, (Block) (flag ? pupmod.a : pupmod.portal4), 0,
							2);
				}
			}
